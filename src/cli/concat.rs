...
pub fn concat_fqstq_lane(
    ...
    for r1 in fp1.lines().map_while(Result::ok) {  vec1.push(r1); }
    for r2 in fp2.lines().map_while(Result::ok) { vec2.push(r2);  }
    if vec1.len() != vec2.len() {
        error!("the number of fastq files in {} and {} is not equal",r1_list, r2_list);
        std::process::exit(1);
    }
